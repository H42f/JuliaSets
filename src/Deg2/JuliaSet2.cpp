...
inline std::complex<double> square (std::complex<double> &z, const std::complex<double> &c) {
...
int main(int argc, char *argv[]) {
   ...
   is.close();
    ...
    DEBUG(std::cout << -x_lim + step_size * (0.5 + doubles_written / (4 * resolutions[resolution])) << ' ' << x_lim << '\n'
                    << -y_lim + step_size * (0.5 + doubles_written % (4 * resolutions[resolution])) << ' ' << y_lim << '\n';)
    ...
    if (os.good()) {
        for (double i = -x_lim + step_size * (0.5 + doubles_written / y_pixels); i < x_lim; i += step_size) {
            for (double j = -y_lim + step_size * (0.5 + doubles_written % y_pixels); j < y_lim; j += step_size) {
                ...
                if (absolute > offset) next = std::log((double) k + std::log2(maxlog / std::log(absolute)));
                else next = 0.;
                os.write((char *) &next, sizeof(double));
                DEBUG(doubles_written++; std::cout << doubles_written << " doubles, i = " << i << ", j= " << j << std::endl;)
                DEBUG(if (!(doubles_written % 100000)) std::cout << doubles_written << " values have been computed in total.\n" << std::endl;)
            }
            DEBUG(std::cout << "next starting j: " << -y_lim + step_size * (0.5 + doubles_written % (4 * resolutions[resolution])) << std::endl;)
        }
    }
