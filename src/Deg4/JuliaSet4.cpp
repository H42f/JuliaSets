...
constexpr std::size_t resolutions[]   = {100, 500, 1000, 5000,10000,
                                         320, 480, 640,
                                         960, 1920, 640};
...
inline std::complex<double> quartic (std::complex<double> &z, const std::complex<double> &c) {
    ...
    if (os.good()) {
        for (double i = -x_lim + step_size * (0.5 + doubles_written / y_pixels); i < x_lim; i += step_size) {
            for (double j = -y_lim + step_size * (0.5 + doubles_written % y_pixels); j < y_lim; j += step_size) {
                ...
                if (absolute > offset) next = std::log((double) k + 0.5 * std::log2(maxlog / std::log(absolute)));
                else next = 0.;
